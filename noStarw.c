...
#define TIMESPEED 0.05                // less == faster

pthread_mutex_t mutexR;            // mutex for blocking the number of readers
pthread_mutex_t mutex;             // mutex for blocking the library and the number of writers
pthread_mutex_t mutexQW;           // mutex for blocking the writers' queue
pthread_mutex_t mutexQR;           // mutex for blocking the readers' queue
...
int *check = NULL;                 // array to check the number of entries of a person
...
void *reader(void *arg) {
    int i = *(int *) arg; // convert and save thread id (void*) to i (int)
    free(arg); // free the allocated space for the arg parameter by main
    int seed;
    time_t tt;
    seed = time(&tt);
    srand(seed);
    while (end == 0) {
        ...
        if (W > 0)
            turn = 1;
        pthread_cond_signal(&condR); // send signal about leaving the library
...
void *writer(void *arg) {
    ...
    while (end == 0) {
        ...
        while ((coutR > 0 || turn == 0) && R > 0 && end == 0)
            pthread_cond_wait(&condR, &mutex); // wait for signal about change in number of readers in the library
        coutW++;
        pthread_mutex_lock(&mutexQW);
        queueW--;
        printf("ReaderQ: %d WriterQ: %d [in: R:%d W:%d]\t%d\n", queueR, queueW, coutR, coutW, i);
        ...
        pthread_cond_broadcast(
                &cond); // send signal to all waiting readers after changing the number of writers in the library
        pthread_mutex_lock(&mutexcheck);
        check[i]++;
        pthread_mutex_unlock(&mutexcheck);
        usleep((rand() % 8000000 + 5000000) * TIMESPEED);
...
int main(int argc, char *argv[]) {
    ...
    for (i = 0; i < W + R; i++) { // initialize arrays
        check[i] = 0;  // initialize the array checking number of entries
    ...
    for (i = 0; i < W; i++) { // creating writer threads
    ...
    for (i = W; i < W + R; i++) { // creating reader threads
    ...
    for (i = 0; i < W + R; i++) { // starting threads
    ...
    free(check);        // clear the occurrences array
    free(tab);            // clear the thread array
    return 0;
}
